        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    @permission(['MedicineType Add','All'])
                   <a href="#addUser" data-toggle="modal" class="btn btn-primary">Add Medicine Type</a>
                   @endpermission
                </div>
                            ...
                            <table class="table table-striped dataex-html5-selectors">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $type->medicines->count() }}</td>

                                        <td>
                                            @permission(['MedicineType Edit','All'])
                                            <button class="btn btn-info btn-sm loadCat" id="{{$type->id}}"><i class="fa fa-pencil"></i></button>
                                            @endpermission
                                            @permission(['MedicineType Delete','All'])
                                            <form id="deleteCategory" style="display:inline">
                                                @csrf
                                                @method('delete')
                                                  <input type="hidden" name="id" id="deleteR" value="{{ $type->id }}">
                                                  <button type="button" id="{{ $type->id }}" class="btn btn-danger btn-sm deleteCat"><i class="fa fa-trash-o"></i></button>
...
<div class="modal fade text-left" id="addUser" tabindex="-1" role="dialog" aria-labelledby="myModalLabel17" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel17">Add Medicine Type</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
    ...
    $('.addCate').click(function () {
        ...
        $.ajax({
            ...
            success:function (response) {
                toastr.success('Medicine Type created successfully.', 'Medicine Type created successfully!', { "showMethod": "fadeIn", "hideMethod": "fadeOut", timeOut: 2000 });
    ...
    $('.editCate').click(function () {
        ...
        $.ajax({
            ...
            success:function (response) {
                ...
                toastr.success('Medicine Type updated successfully.', 'Medicine Type updated successfully!', { "showMethod": "fadeIn", "hideMethod": "fadeOut", timeOut: 2000 });
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if(response.success == true) {
                            ...
                            Swal.fire({
                                title: 'Error Deleting Medicine Type',
                                text:'This Medicine Type is already associated with some medicines',
                                animation: false,
